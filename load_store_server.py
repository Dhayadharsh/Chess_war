    ...
    def get_movement_moves(self, board, row, col):
        ...
        for direction in directions:
                ...
                if 0 <= new_row < BOARD_SIZE and 0 <= new_col < BOARD_SIZE:
                    ...
                    if board[new_row-direction[0]*(direction[0]!=0)][new_col-direction[1]*(direction[1]!=0)] == None and  board[new_row][new_col] is None:
                        valid_moves.append((new_row, new_col))
        return valid_moves
    def get_valid_moves(self, board, row, col):
        valid_moves = []
...
class Pawn(Piece):
    ...
    def get_valid_moves(self, board, row, col):
        ...
        if 0 <= row + self.direction < BOARD_SIZE and board[row + self.direction][col] is None:
            ...
            if not self.has_moved and 0 <= row + 2 * self.direction < BOARD_SIZE and board[row + 2 * self.direction][col] is None:
...
class ChessBoard:
    ...
    def draw(self, screen, selected_piece, valid_moves,movement_moves,group_move,p):
        ...
        if self.is_in_check(PWHITE):
            ...
            pygame.draw.rect(screen, CHECK_COLOR, pygame.Rect(king_pos[1] * SQUARE_SIZE, king_pos[0] * SQUARE_SIZE, SQUARE_SIZE, SQUARE_SIZE), 5)
...
def draw_midline(screen):
    for col in range(BOARD_COLS):
        pygame.draw.line(screen, MIDLINE_color, (col * SQUARE_SIZE, BOARD_ROWS // 2 * SQUARE_SIZE), ((col + 1) * SQUARE_SIZE, BOARD_ROWS // 2 * SQUARE_SIZE), 2)
...
def black_placer():
    ...
    pygame.display.set_caption("Black Player Placement")
    board = ChessBoard()
    selected_piece = None
    running = True
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if x >= BOARD_COLS * SQUARE_SIZE:
                    selected_piece = [King(BLACK), Knight(BLACK), swordsman(BLACK), Pawn(BLACK, pc), Bishop(BLACK), Queen(BLACK), Rook(BLACK)][(y // SQUARE_SIZE) % 7]
                elif y// SQUARE_SIZE<=6:
                    row, col = y // SQUARE_SIZE, x // SQUARE_SIZE
                    if selected_piece and event.button == 1:
                        if isinstance(selected_piece, Pawn) and pc < PC:
                            board.place_piece(Pawn(BLACK, pc), (row, col))
                            pc += 1
                            piece_counts[3] = pc
                        elif isinstance(selected_piece, King) and kc < KC:
                            board.place_piece(King(BLACK), (row, col))
                            kc += 1
                            piece_counts[0] = kc
                        elif isinstance(selected_piece, Queen) and qc < QC:
                            board.place_piece(Queen(BLACK), (row, col))
                            qc += 1
                            piece_counts[5] = qc
                        elif isinstance(selected_piece, Rook) and rc < RC:
                            board.place_piece(Rook(BLACK), (row, col))
                            rc += 1
                            piece_counts[6] = rc
                        elif isinstance(selected_piece, Bishop) or isinstance(selected_piece, Knight) or isinstance(selected_piece, swordsman) and oc < OC:
                ...
                if event.key == pygame.K_k:
                    selected_piece = King(BLACK)
                elif event.key == pygame.K_n:
                    selected_piece = Knight(BLACK)
                elif event.key == pygame.K_s:
                    selected_piece = swordsman(BLACK)
                elif event.key == pygame.K_p:
                    selected_piece = Pawn(BLACK, pc)
                elif event.key == pygame.K_b:
                    selected_piece = Bishop(BLACK)
                elif event.key == pygame.K_q:
                    selected_piece = Queen(BLACK)
                elif event.key == pygame.K_r:
                    selected_piece = Rook(BLACK)
...
def board_combiner(wboard,bboard):
    combined=list(wboard)
    for row in range(BOARD_ROWS//2):
            for col in range(BOARD_COLS):
                combined[row][col]=(bboard[row][col])

    for row in range(BOARD_ROWS//2,BOARD_ROWS):
            for col in range(BOARD_COLS):
                combined.append(wboard[row][col])       
    return combined     
...
HOST = "172.23.5.46"  # Server IP address (same as the client)
PORT = 12346
import pygame

# ChessBoard and other necessary classes should be defined here or imported.
...
def run_server():
                ...
                if pawn.color==turn_color:
                    ...
                    for pos1 in pawn_pos:
                        if pos in adjacent_pos(pos1):
                            poscheck=True 
                    if poscheck:
                        if ((pos[0] + pawn.direction, pos[1])) in (chess_board.get_valid_moves(pawn,pos[0],pos[1])):
                            if ((act_pos[0] + act_pawn.direction, act_pos[1])) in (chess_board.get_valid_moves(act_pawn,act_pos[0],act_pos[1])):
                                    boole=True
            return boole
    ...
    result = load_window()
    if result == 'new':
        # Proceed to the second window for piece placement

        bboard = black_placer()
        bboard = bboard.tolist() 
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        with client_socket:
            board_json = client_socket.recv(4096).decode('utf-8')
            print("recieved")
            wboard = json.loads(board_json)
                 
             
            client_socket.sendall(json.dumps(bboard).encode('utf-8'))
            print("sent")
              # White player's board received
            ...
            pygame.display.set_caption("16x16 Chess Board (Server)")
